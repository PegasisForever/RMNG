...
function account(
  ...
  return {
    ...
    ...(sevenDay === undefined
      ? {}
      : { sevenDay: { pct: sevenDay, resetsAt: null } }),
  };
}

test("no data means no peak", () => {
...
test("the highest window across all accounts wins, and it names the account", () => {
  const peak = peakUsage([
    account("alex@example.com", 42, 61),
    account("sam@example.com", 88, 73),
  ]);
...
test("the Fable window never counts, since it gates nothing", () => {
  ...
  expect(peakUsage([fableHeavy])).toEqual({
    pct: 20,
