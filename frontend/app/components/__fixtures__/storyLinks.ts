 ...
 *  its display name (`"Agent working"`). Call it from inside a handler when the destination
 *  depends on the callback's own argument; otherwise use `makeStoryLink`. */
export function linkToStory(title: string, exportName: string): void {
  const name = exportName.includes(" ") ? exportName : storyNameFromExport(exportName);
  navigate({ storyId: toId(title, name) });
}

/** Build a callback that jumps to another story when it fires. Drops straight into story args
 *  in place of `fn()` wherever the prop is a navigation seam:
 *
 *  ```ts
 *  onOpenSettings: makeStoryLink("Settings/SettingsPanel", "Default")
 *  ```
 *
...
export function makeStoryLink(title: string, exportName: string): (...args: unknown[]) => void {
  return () => linkToStory(title, exportName);
}
