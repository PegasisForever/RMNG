...
//
// Pure: it takes presets and a create function, so a story can drive it with a spy.
...
// BlockNote is browser-only and heavy; the description field pulls it in on demand. Same
// editor the clone dialog's own description uses.
...
/** Every distinct team key across the presets' labels, each with the preset that claims it:
 *  the first in config order, mirroring the server's `pick_preset_by_prefix`. */
function teamKeysOf(presets: PresetRedacted[]) {
  const seen = new Map<string, PresetRedacted>();
  for (const preset of presets) {
    for (const label of preset.labels) {
      const key = label.toLowerCase();
      if (!seen.has(key)) seen.set(key, preset);
    }
  }
  return [...seen.entries()].map(([key, preset]) => ({ key, preset }));
}

export function TicketModal({ presets, onClose, onCreate }: TicketModalProps) {
