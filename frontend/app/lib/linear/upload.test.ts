...
function answer(overrides: Record<string, unknown> = {}): unknown {
  return {
    fileUpload: {
      ...
      uploadFile: {
        uploadUrl:
          "https://storage.googleapis.com/linear-uploads/abc?X-Goog-Expires=60",
...
test("a refusal, or an answer missing either URL, throws rather than uploading nowhere", () => {
  expect(() =>
    uploadTargetFromResponse({ fileUpload: { success: false } }),
  ).toThrow();
  expect(() => uploadTargetFromResponse(null)).toThrow();
  expect(() => uploadTargetFromResponse(answer({ uploadUrl: null }))).toThrow(
    /uploadUrl/,
  );
  expect(() => uploadTargetFromResponse(answer({ assetUrl: "" }))).toThrow(
    /assetUrl/,
  );
});
...
test("a nameless header is dropped", () => {
