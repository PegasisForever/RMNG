...
//
// The move to In Progress used to be the last of the Linear calls the clone route made
// server-side. The browser holds every preset key now, so it does both itself and posts the
// answer with the fork payload. What crossed over is the part the function names do not say:
//
...
async function firstKeyThatSees<T>(
 ...
 throw (
  last ??
  new Error("no preset has a Linear API key configured. Add one in Settings")
 );
}
...
export async function fetchTicketAny(
 keys: string[],
 ref: IssueRef,
): Promise<LinearTicket> {
...
export async function ensureInProgress(
 ...
 if (issue.uuid === "")
  throw new Error(`${issue.identifier} arrived without a Linear id`);

 const team = issue.prefix.toUpperCase();
 const data = await gql<{ teams?: { nodes?: unknown[] } }>(
  key,
  TEAM_STATES_QUERY,
  { team },
 );
 ...
 if (stateId === null)
  throw new Error(`no "In Progress" state found for team ${team}`);
...
function names(nodes: unknown): string[] {
 ...
 for (const node of nodes) {
  const name = obj(node);
  if (name && typeof name.name === "string" && name.name !== "")
   kept.push(name.name);
 }
 return kept;
}
