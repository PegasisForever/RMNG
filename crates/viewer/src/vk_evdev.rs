...
//! directly (`hardware_keycode = evdev + 8`); the macOS twin is [`crate::kvk_evdev`], which
...
//! than send 0, exactly as with [`crate::kvk_evdev`].
//!
...
mod tests {
    ...
    /// entries most likely to rot. `kvk_evdev` maps the same evdev codes from the macOS side —
    /// these values must agree with it or the same physical key means two things per platform.
    #[test]
