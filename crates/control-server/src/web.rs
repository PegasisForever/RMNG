...
struct TestReq {
    ...
    #[serde(default)]
    value: String,
    /// Same idea for the judge provider: `"judge"` uses `value` as the unsaved Gemini key
    /// and this as the unsaved provider (`"codex"` / `"gemini"`, empty = stored).
    #[serde(default)]
    model: String,
}
...
/// `"judge"` puts one real stuck-detection question to the picked backend.
async fn config_test(State(app): State<App>, Json(req): Json<TestReq>) -> Json<serde_json::Value> {
    let (ok, message) = match req.what.as_str() {
        ...
        "judge" => crate::stuck::probe_judge(&app, &req.model, &req.value).await,
