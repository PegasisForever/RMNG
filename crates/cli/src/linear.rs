//! Linear calls the ticket-driven clone verbs make for themselves.
//!
//! Talks to `api.linear.app/graphql` with a personal API key (`Authorization: <key>`, no
//! "Bearer"). The keys come from `GET /api/config`, where every preset carries its own
//! ([`wire::PresetRedacted::linear_key`]), so an existing ticket is fetched by trying each
//! preset's key ([`fetch_issue_any`]) and the ticket-id prefix picks the preset
//! ([`pick_preset_by_prefix`]). The ticket-id prefix (e.g. `WE-142` → `we`) names the team
//! within whichever workspace the key can see.
//!
//! The clone itself is still the control-server's to start: this resolves the issue and
//! `POST /api/clone` is handed the answer.
...
mod tests {
    ...
    fn picks_preset_by_ticket_prefix() {
        let p = |name: &str, labels: &[&str]| wire::PresetRedacted {
            name: name.into(),
